<layouts>
    <div class="pb-24">
                    ...
                    <label class="flex flex-col min-w-40 h-14 w-full">
                        <div class="flex w-full flex-1 items-stretch rounded-full h-full bg-white shadow-sm">
                            <div class="text-[#756783] flex items-center justify-center pl-5">
                                <x-lucide-search class="text-[#756783] w-8 h-8" />
                            </div>
                    ...
                    @forelse($products as $product)
                        <div class="flex flex-col gap-3 group">
                            ...
                            <div
                                ...
                                <!-- Info Produk -->
                                <div class="px-4 pb-4">
                                    <p class="text-[#141217] text-sm font-bold leading-tight line-clamp-2 truncate">
                                        {{ $product->nama_produk }}
                                    </p>
        ...
        <div x-data="{ open: @entangle('showModal') }" x-show="open" x-on:keydown.escape.window="open = false"
            ...
            class="fixed inset-0 z-150 flex items-end justify-center bg-black bg-opacity-50 overflow-hidden" x-cloak>
            ...
            <div x-show="open" x-transition:enter="ease-out duration-300"
                ...
                <form wire:submit.prevent="saveProduct" class="px-6 space-y-6 mt-6 mb-6" id="product-form">
                    ...
                    <div>
                        ...
                        @error('isi_per_box') <span class="text-red-500 text-xs mt-1">{{ $message }}</span> @enderror
                    </div>
                    ...
                    <div>
                        ...
                        @if($this->harga_jual_per_item && $this->isi_per_box && $this->hpp)
                            <p class="text-xs text-gray-500 mt-1">
                                ({{ $this->isi_per_box }} × Rp {{ number_format($this->harga_jual_per_item, 0, ',', '.') }})
                        ...
                        <div class="border-2 border-dashed border-gray-300 rounded-2xl p-4 text-center hover:border-purple-400 transition cursor-pointer"
                            ...
                            <div class="flex flex-col items-center space-y-2">
                                @if($foto)
                                    ...
                                    <p class="text-sm text-purple-700 font-medium">{{ $foto->getClientOriginalName() }}</p>
                                @elseif($fotoPath)
                                    <!-- Tampilkan foto lama -->
